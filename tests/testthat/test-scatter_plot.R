...
testthat::test_that("Function can only accept certain data types for arguments", {
  expect_error(scatter_plot(c(1,0), c(1,0), c(0,1), "x", "y", "title", 10, NULL))
  expect_error(scatter_plot(test_data_scatter2, x, y, "x", "y", "title", 10, "lm"))
  expect_error(scatter_plot(test_data_scatter3, x, y, "x", "y", "title", 10, NULL))
  expect_error(scatter_plot(test_data_scatter, x, y, x, "y", "title", 10, NULL))
  expect_error(scatter_plot(test_data_scatter, x, y, "x", y, "title", 10, NULL))
  expect_error(scatter_plot(test_data_scatter, x, y, "x", "y", 10, 10, NULL))
  expect_error(scatter_plot(test_data_scatter, x, y, "x", "y", "title", "10", NULL))
  expect_error(scatter_plot(test_data_scatter, x, y, "x", "y", "title", 10, "NULL"))
  expect_error(scatter_plot(test_data_scatter, x, y, "x", "y", "title", 10, lm))
})

#test type of scatter_plot
testthat::test_that("Function has the right type", {
  expect_equal(helper_scatter_empty_type$type, NULL)
  expect_equal(helper_scatter_lm_type$type,"lm")
})
...
testthat::test_that("Checks the output of the graph", {
  plot1 <- scatter_plot(test_data_scatter, x, y, "x", "y", "Scatterplot Test", 10, NULL)
  ...
  expect_equal(ggplot2::plot1$type, NULL)
})

#test for output variables for function with type="lm"
testthat::test_that("Checks the output of the graph", {
  plot2 <- scatter_plot(test_data_scatter, x, y, "x", "y", "Scatterplot Test", 10, "lm")
