...
helper_scatter_knn_type <- ggplot2::ggplot(test_data_scatter,ggplot2::aes(x=x,y=y)) +
  ggplot2::geom_point()+
  ggplot2::labs(x="xlab",y="ylab")+
  ggplot2::ggtitle("Scatterplot Test")+
  ggplot2::theme(text=ggplot2::element_text(size=10))+
  ggplot2::geom_smooth(method = "knn", se = FALSE) 
...
testthat::test_that("Checks the output of the graph", {
  plot2 <- scatter_plot(test_data_scatter, x, y, "x", "y", "Scatterplot Test", 10)
...
testthat::test_that("Function has the right type", {
  ...
  expect_type(helper_scatter_lm_type ,'list')
  expect_type(helper_scatter_knn_type ,'list')
})
